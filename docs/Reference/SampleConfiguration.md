...
Set the `mode` to `orion` to use Tessera as the privacy manager when using [Hyperledger Besu versions prior to 25.1.0 in non-GoQuorum mode](https://besu.hyperledger.org/en/stable/HowTo/Use-Privacy/Use-GoQuorum-compatible-privacy/). [Enabling this mode](../HowTo/Configure/Orion-Mode.md) changes Tessera's behavior. This property is optional.

### `useWhiteList`

Use the `useWhiteList` field to restrict connections to Tessera to specified peers. If set to `true`, then only nodes listed in the [`peer`](#peer) list are allowed to connect.
...
Defines an optional [remote enclave](../Concepts/Privacy-Manager/Enclave-types.md#remote-http-enclave). Leave out if using a [local enclave](../Concepts/Privacy-Manager/Enclave-types.md#local-enclave).
...
The peer-to-peer (P2P) [server](../HowTo/Configure/TesseraAPI.md) is used to perform discovery and send and receive encrypted payloads.
...
The Quorum-to-Tessera (Q2T) [server](../HowTo/Configure/TesseraAPI.md) is used to check if the Tessera node is running, and to send and receive private transactions.
...
Configure [cross-origin resource sharing (CORS)](../HowTo/Configure/TesseraAPI.md#configure-cors) to control access to resources outside the domain.
